...
import { editarClima, buscarClimaPorId } from '../../services/climaServices';
...
export default function CadastrarDadosMeteorologicos(){   
  ...
  useEffect(() => {
    ...
    async function carregarDados() {
      try {
        const response = await buscarClimaPorId(Number(id));

        setForm({
          cidade: response.cidade,
          data: response.data,
          tempoDia: response.tempoDia,
          tempoNoite: response.tempoNoite,
          turnoDia: 'Dia',
          turnoNoite: 'Noite',
          tempMaxima: response.tempMaxima.toString(),
          tempMinima: response.tempMinima.toString(),
          precipitacao: response.precipitacao.toString(),
          humidade: response.humidade.toString(),
          velocidadeVento: response.velocidadeVento.toString(),
        });
  ...
  const salvarDados = async () => {
    ...
    try {
      await editarClima(Number(id), payload);
      toast.success('Dados atualizados com sucesso!');
      navigate('/listar-dados');
    } catch (error: any) {
    ...
    return(
      ...
      <div className="actions">
        ...
        <button className="button save" onClick={salvarDados}>
          Salvar
        </button>
      </div>
      <Footer />
